...
{
  return Hermes::atan(slope * (Hermes::sqrt(sqr(x-1.25) + sqr(y+0.25)) - M_PI/3));
}

void CustomExactSolution::derivatives (double x, double y, double& dx, double& dy) const 
{
    double t = Hermes::sqrt(sqr(x - 1.25) + sqr(y + 0.25));
...
double CustomFunction::value(double x, double y) const 
{
  ...
  return (   27.0/2.0 * sqr(2.0*y + 0.5) * (M_PI - 3.0*t) * Hermes::pow(slope, 3.0) / (sqr(u) * t2) 
            + 27.0/2.0 * sqr(2.0*x - 2.5) * (M_PI - 3.0*t) * Hermes::pow(slope, 3.0) / (sqr(u) * t2) 
            - 9.0/4.0 * sqr(2.0*y + 0.5) * slope / (u * Hermes::pow(t,3.0)) 
            - 9.0/4.0 * sqr(2.0*x - 2.5) * slope / (u * Hermes::pow(t,3.0)) + 18.0 * slope / (u * t)
          );
}
